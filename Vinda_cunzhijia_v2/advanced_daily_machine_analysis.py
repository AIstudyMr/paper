...
def analyze_stop_patterns(stopped_data, cuts_col):
    """分析停机模式"""
    ...
    stopped_data['time_diff'] = stopped_data['时间'].diff().dt.total_seconds()
    
    # 识别停机时段（间隔超过60秒认为是新的停机时段）
    stop_segments = []
    current_segment = []
    ...
    for idx, row in stopped_data.iterrows():
        if len(current_segment) == 0 or row['time_diff'] <= 60:
            ...
            if len(current_segment) > 10:  # 至少10个数据点才算一个有效停机时段
                stop_segments.append(pd.DataFrame(current_segment))
            current_segment = [row]
    ...
    # 分析每个停机时段
    segment_stats = []
    for i, segment in enumerate(stop_segments):
...
def plot_enhanced_daily_curves(df, machine_number, date, base_folder):
    """绘制增强版每日曲线图"""
    # 设置图表样式
    ...
    try:
        ...
        # 分析停机模式
        stop_analysis = analyze_stop_patterns(stopped_data, cuts_col)
        ...
        # 主标题
        fig.suptitle(f'{machine_number}号小包机 {date} 停机时瞬时切数与内外循环分析', 
        ...
        ax1.fill_between(stopped_data['时间'], stopped_data[cuts_col], 
                        alpha=0.3, color='#2E86AB')
        
        ax1.set_ylabel('瞬时切数 (个/秒)', fontsize=12, fontproperties=FONT_PROP)
        ax1.set_title('停机时瞬时切数时间序列', fontsize=14, fontweight='bold', fontproperties=FONT_PROP)
        ...
        if inner_loop_col in df.columns:
            ...
            ax2.fill_between(stopped_data['时间'], stopped_data[inner_loop_col], 
                            alpha=0.3, color='#F18F01')
            ax2.set_ylabel('外循环进内循环纸条数量', fontsize=12, fontproperties=FONT_PROP)
            ax2.set_title('停机时外循环进内循环时间序列', fontsize=14, fontweight='bold', fontproperties=FONT_PROP)
        else:
        ...
        if inner_loop_col in df.columns:
            inner_loop_data = stopped_data[inner_loop_col].dropna()
        ...
        if stop_analysis['stats']:
            segments_df = pd.DataFrame(stop_analysis['stats'])
            bars = ax5.bar(range(len(segments_df)), segments_df['持续时间(分钟)'], 
                          color='#C73E1D', alpha=0.7)
            ax5.set_xlabel('停机时段编号', fontsize=12, fontproperties=FONT_PROP)
            ax5.set_ylabel('持续时间 (分钟)', fontsize=12, fontproperties=FONT_PROP)
            ax5.set_title('停机时段持续时间分析', fontsize=14, fontweight='bold', fontproperties=FONT_PROP)
            ...
            # 保存停机时段数据为CSV
            csv_filename = f"{machine_number}号小包机_{date}_停机时段分析.csv"
            ...
            print(f"  停机时段数据已保存至: {csv_filepath}")
        else:
            ax5.text(0.5, 0.5, '未检测到明显的停机时段模式', 
                    transform=ax5.transAxes, ha='center', va='center', fontsize=14, fontproperties=FONT_PROP)
            ax5.set_title('停机时段分析', fontsize=14, fontweight='bold', fontproperties=FONT_PROP)
        ...
        # 创建统计信息
        stats_data = [
            ['总停机数据点数', f'{len(stopped_data):,}'],
            ['平均瞬时切数', f'{stopped_data[cuts_col].mean():.3f}'],
            ['最大瞬时切数', f'{stopped_data[cuts_col].max():.3f}'],
            ['最小瞬时切数', f'{stopped_data[cuts_col].min():.3f}'],
            ['瞬时切数标准差', f'{stopped_data[cuts_col].std():.3f}'],
            ['瞬时切数中位数', f'{stopped_data[cuts_col].median():.3f}']
        ]
        
        if inner_loop_col in df.columns:
            stats_data.extend([
                ['平均外循环进内循环', f'{stopped_data[inner_loop_col].mean():.3f}'],
                ['最大外循环进内循环', f'{stopped_data[inner_loop_col].max():.3f}'],
                ['最小外循环进内循环', f'{stopped_data[inner_loop_col].min():.3f}'],
                ['外循环进内循环标准差', f'{stopped_data[inner_loop_col].std():.3f}']
            ])
        ...
        # 保存图表
        filename = f"{machine_number}号小包机_{date}_瞬时切数与内外循环分析.png"
        filepath = os.path.join(base_folder, f"{machine_number}号小包机", filename)
        plt.savefig(filepath, dpi=300, bbox_inches='tight', facecolor='white')
        plt.close()
        
        # 打印详细统计信息
        print(f"\n{machine_number}号小包机 {date} 详细停机统计:")
        print(f"  停机数据点数: {len(stopped_data):,}")
        print(f"  瞬时切数统计: 平均={stopped_data[cuts_col].mean():.3f}, "
              f"最大={stopped_data[cuts_col].max():.3f}, "
              f"标准差={stopped_data[cuts_col].std():.3f}")
        
        if inner_loop_col in df.columns:
            print(f"  外循环进内循环统计: 平均={stopped_data[inner_loop_col].mean():.3f}, "
                  f"最大={stopped_data[inner_loop_col].max():.3f}, "
                  f"标准差={stopped_data[inner_loop_col].std():.3f}")
        
        print(f"  检测到 {len(stop_analysis['stats'])} 个明显的停机时段")
        print(f"  瞬时切数与内外循环分析图表已保存至: {filepath}")
        
        return True
        
    except Exception as e:
        print(f"绘制 {machine_number}号小包机 {date} 增强图表时出错: {e}")
        return False

def create_machine_comparison(df, date, base_folder):
    """创建多机器对比分析"""
    try:
        # 筛选指定日期的数据
        date_start = pd.Timestamp(date)
        ...
        fig.suptitle(f'{date} 四台小包机瞬时切数与内外循环对比分析', fontsize=16, fontweight='bold', fontproperties=FONT_PROP)
        ...
        for patch, color in zip(bp1['boxes'], colors):
            patch.set_facecolor(color)
            patch.set_alpha(0.7)
        ax1.set_title('瞬时切数分布对比', fontweight='bold', fontproperties=FONT_PROP)
        ...
        ax2.set_title('平均瞬时切数对比', fontweight='bold', fontproperties=FONT_PROP)
        ...
        ax3.set_title('停机数据点数对比', fontweight='bold', fontproperties=FONT_PROP)
        ...
        if inner_loop_col in df.columns:
            ...
            if inner_loop_machines:
                ...
                ax4.set_title('平均外循环进内循环对比', fontweight='bold', fontproperties=FONT_PROP)
        ...
        # 保存对比图表
        comparison_filename = f"{date}_四台小包机瞬时切数与内外循环对比.png"
        ...
        print(f"  多机器对比分析图表已保存至: {comparison_filepath}")
        return True
        
    except Exception as e:
        print(f"创建 {date} 多机器对比分析时出错: {e}")
        return False

def create_daily_segments_summary(df, date, base_folder):
    """创建每日所有机器停机时段汇总"""
    try:
        # 筛选指定日期的数据
        date_start = pd.Timestamp(date)
        ...
        for machine_number in range(1, 5):
            ...
            if speed_col in df.columns and cuts_col in df.columns:
                ...
                if not stopped_data.empty:
                    ...
                    if stop_analysis['stats']:
                        for segment_stat in stop_analysis['stats']:
        ...
        if all_segments:
            ...
            # 保存汇总CSV
            summary_filename = f"{date}_所有机器停机时段汇总.csv"
            ...
            print(f"  当日所有机器停机时段汇总已保存至: {summary_filepath}")
            print(f"  共检测到 {len(all_segments)} 个停机时段")
            return True
        
        return False
        
    except Exception as e:
        print(f"创建 {date} 停机时段汇总时出错: {e}")
        return False

def main():
    """主函数"""
    print("开始生成瞬时切数与内外循环分析...")
    ...
    while current_date <= end_date:
        ...
        # 为每个小包机绘制瞬时切数与内外循环分析图表
        for machine_number in range(1, 5):
            total_attempts += 1
            success = plot_enhanced_daily_curves(df, machine_number, current_date, base_folder)
            if success:
                total_success += 1
        
        # 创建多机器对比分析
        create_machine_comparison(df, current_date, base_folder)
        
        # 创建当日所有机器停机时段汇总
        create_daily_segments_summary(df, current_date, base_folder)
