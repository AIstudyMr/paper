...
def smooth_paper_rate(data, smooth_window, method='moving_average'):
    """
    对存纸率数据进行平滑处理
...
def calculate_detailed_slope_stats(paper_data, start_time, end_time, window_size, smooth_window, smooth_method='moving_average'):
    """
    使用滑动窗口计算详细的斜率统计信息
    ...
    if len(non_zero_slopes) == 0:
        ...
        return {
            ...
            'min_slope': 0,
            'slope_range': 0,
            'window_count': len(slopes)  # 保持原始窗口数量
        }
...
def main():
    print("开始分析折叠机正常运行时间段内的小包机停机状态...")
    print("注意：已启用存纸率平滑处理功能（移动平均，窗口大小=5）")
    
    # 读取折叠机停机数据
    try:
        folding_downtime = read_csv_with_encoding('折叠机正常运行时间段.csv')
    ...
    result_df.to_csv('折叠机运行分析结果_多状态_30.csv', index=False, encoding='utf-8-sig')
    print(f"\n分析完成！结果已保存到 '折叠机运行分析结果_多状态.csv'")
