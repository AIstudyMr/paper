...
def analyze_folding_stop_results():
    """分析折叠机停机时间段内小包机停机分析结果（滑动窗口方法）"""
    
    # 读取结果文件
    try:
        df = pd.read_csv('折叠机停机分析结果_25_多状态_30.csv', encoding='utf-8-sig')
        print("成功读取折叠机停机分析结果文件")
        print(f"数据行数: {len(df)}")
    ...
    print(f"基准斜率（折叠机停机期间，无小包机停机）: {baseline_slope:.4f} /分钟")
    ...
    # 保存到CSV
    output_filename = '折叠机停机期间停机组合影响分析汇总_30.csv'
...
def analyze_slope_stability():
    ...
    elif '平均斜率' in valid_data.columns:
        valid_data['斜率数值'] = valid_data['平均斜率']
    else:
    ...
    # 保存稳定性分析结果
    stability_analysis.to_csv('折叠机停机期间斜率稳定性分析_30.csv', encoding='utf-8-sig')
    print(f"\n斜率稳定性分析结果已保存到 '折叠机停机期间斜率稳定性分析_30.csv'")
...
def analyze_slope_statistics_by_combination():
    ...
    # 保存详细统计结果
    output_filename = '折叠机停机期间小包机停机组合斜率详细统计分析_30.csv'
...
def analyze_all_combinations_slope_statistics():
            ...
            try:
                ...
                if '斜率:' in combination and '%' in combination:
                    ...
                    if len(parts) >= 2:
                        ...
                        # 适配停机版本的字段结构
                        time_segment_slope = row.get('全段平均斜率', row.get('主要组合斜率', row.get('平均斜率', 0)))
                        
                        all_combinations_data.append({
                            '时间段开始': row['开始时间'],
                            '时间段结束': row['结束时间'],
                            '时间段持续': row['持续时间'],
                            '主要停机组合': row.get('主要停机组合', row.get('停机组合', '')),
                            '停机组合名称': combination_name,
                            '时间占比(%)': percentage,
                            '斜率': slope,
                            '时间段总点数': row['总时间点数'],
                            '时间段全段平均斜率': time_segment_slope
                        })
            except Exception as e:
    ...
    # 保存详细统计结果
    output_filename = '折叠机停机期间所有停机组合斜率详细统计分析_30.csv'
...
if __name__ == "__main__":
    print("开始进行折叠机停机时间段内小包机停机状态的详细分析...")
    df, detailed_df = analyze_folding_stop_results()
    ...
    print(f"\n=== 分析完成摘要 ===")
    print(f"✓ 基础分析结果: '折叠机停机期间停机组合影响分析汇总_30.csv'")
    print(f"✓ 稳定性分析结果: '折叠机停机期间斜率稳定性分析_30.csv'")
    print(f"✓ 详细斜率统计结果: '折叠机停机期间小包机停机组合斜率详细统计分析_30.csv'")
    print(f"✓ 所有停机组合斜率统计: '折叠机停机期间所有停机组合斜率详细统计分析_30.csv'")
